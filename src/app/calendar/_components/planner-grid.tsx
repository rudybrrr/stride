    ...
    <div className="surface-card overflow-hidden">
      <div className="overflow-x-auto">
        <div className={cn(days.length > 1 && "min-w-[980px]")}>
          <div className="grid border-b border-border/18 bg-background/82" style={{ gridTemplateColumns }}>
            <div className="sticky left-0 z-20 border-r border-border/18 bg-background/82 px-3 py-2 text-[10px] font-semibold tracking-[0.14em] text-muted-foreground">
              Day
            </div>
            {days.map((day) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex min-h-[64px] flex-col items-start justify-center gap-0.5 border-r border-border/18 px-3 py-2 text-left transition-colors last:border-r-0",
                    isSelected ? "bg-primary/8" : "bg-background/24 hover:bg-muted/28",
                  )}
                >
                    ...
                    <span
                      className={cn(
                        ...
                        isSelected
                          ...
                          : "border-border/18 bg-background/76 text-muted-foreground",
                      )}
                    >
                      {itemCount}
                    </span>
          ...
          <div className="grid border-b border-border/18 bg-background/78" style={{ gridTemplateColumns }}>
            <div className="sticky left-0 z-20 border-r border-border/18 bg-background/78 px-3 py-2.5 text-[10px] font-semibold tracking-[0.14em] text-muted-foreground">
              Due tasks
            </div>
            {days.map((day) => {
              const dayKey = toDateKey(day);
              ...
              return (
                <div
                  key={dayKey}
                  className={cn(
                    "min-h-[5.25rem] space-y-1.5 border-r border-border/18 px-2.5 py-2.5 last:border-r-0",
                    isSameDay(day, selectedDate) ? "bg-primary/6" : "bg-background/12",
                  )}
                >
                  {visibleTasks.length > 0 ? visibleTasks.map((task) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex w-full items-center gap-2 rounded-xl border px-2.5 py-2 text-left text-[13px] transition-colors hover:bg-background/92",
                          colors.soft,
                          colors.border,
                        )}
                      >
                        ...
                        <span className="shrink-0 rounded-full border border-border/18 bg-background/76 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                          {project?.name ?? "Project"}
                        </span>
                      </button>
                    );
                  }) : (
                    <div className="flex min-h-[5.25rem] items-center justify-center rounded-xl border border-dashed border-border/70 bg-background/28 text-xs tracking-[0.12em] text-muted-foreground">
                      Clear
                    </div>
                  )}
                  ...
                  {remainingCount > 0 ? (
                    <button
                      type="button"
                      onClick={() => onSelectDate(day)}
                      className="inline-flex items-center rounded-full border border-border/18 bg-background/76 px-2.5 py-0.5 text-[10px] font-semibold tracking-[0.12em] text-muted-foreground transition-colors hover:text-foreground"
                    >
                      +{remainingCount} more
                    </button>
                  ) : null}
          ...
          <div ref={timedGridRef} className="relative grid" style={{ gridTemplateColumns }}>
            <div className="sticky left-0 z-20 border-r border-border/18 bg-background/82">
              {plannerHours.map((hour) => (
                <div
                  key={hour}
                  className="flex items-start justify-end border-b border-border/18 px-3 pt-1.5 text-[11px] font-medium text-muted-foreground/40 last:border-b-0"
            ...
            {days.map((day, dayIndex) => {
              ...
              return (
                <div
                  key={dayKey}
                  className={cn(
                    "relative border-r border-border/18 last:border-r-0 touch-none",
                    isSameDay(day, selectedDate) ? "bg-primary/[0.03]" : "bg-background/8",
                  )}
                  onPointerDown={(event) => {
                ...
                >
                  {plannerHours.map((hour) => (
                    <div key={`${dayKey}-${hour}`} className="border-b border-border/18 last:border-b-0" style={plannerRowStyle} />
                  ))}
                  ...
                  {dayLayouts.map((layout) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "absolute overflow-hidden rounded-[0.95rem] border bg-[var(--surface-elevated)] px-2.5 py-2 text-left shadow-[var(--shadow-xs)] transition-opacity",
                          colors.soft,
                          colors.border,
                          active && "opacity-30",
                        )}
                  ...
                  {previewDayKey === dayKey && interaction ? (
                    <div
                      className={cn(
                        "pointer-events-none absolute inset-x-1 rounded-[0.95rem] border border-dashed bg-primary/10",
                        interaction.type === "create"
                          ? "border-primary/50"
